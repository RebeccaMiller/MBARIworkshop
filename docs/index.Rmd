---
title: "index"
author: "Rebecca"
date: "November 30, 2017"
output:html_document:
code_folding: hide
toc: true
toc_float: true


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## R Markdown

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:


##Scatterplot MPG
```{r cars}
summary(cars)

library(tidyverse)
##data from r
mpg
g <- ggplot(data= mpg, aes(x = displ, y=hwy))   ## g object is 
g+ geom_point()



```

## i want color


```{r}
g + geom_point(aes(color = class))
```


## i want SHAPE


```{r}
g + geom_point(aes(shape = class))
```




##shape size
```{r}
ggplot(data = mpg, aes(x=cty, y= hwy, size= class, color, fl)) + geom_point (alpha =0.2, color = 'blue')
```


## exercise 1

What’s gone wrong with this code?
    ggplot(data = mpg) + 
      geom_point(aes(x = displ, y = hwy, color = "blue"))
      
     
    ggplot(data = mpg) + 
      geom_point(aes(x = displ, y = hwy) color = "blue")
      
      
```{r}
ggplot(data = mpg, aes(x=hwy, y= displ, size= class, color, cty)) + geom_point (alpha =0.2, color = 'blue')

ggplot(data = mpg, aes(x=hwy, y= displ, size= class, color, cty)) + geom_point (alpha =0.2, color =  displ < 5)

```
 
 
 ##Geoms
```{r}

ggplot(mpg, aes(x = drv, y = hwy))+ 
geom_violin() #violin plot

ggplot(data=mpg,aes(x=displ, y =hwy))+
geom_point() +
geom_smooth()+  ##smoothing is loess
labs(title="relationship b/w engine size", x = "hwy mpg", y = " engine displacement") +

theme_bw()+
theme(text = element_text(size = 10))

```



## Including Plots

You can also embed plots, for example:

```{r pressure, echo=FALSE}
plot(pressure)
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.
